      ...
      {cartOpen && (
        <div className="fixed inset-0 z-50 flex justify-end">
          ...
          <motion.div
            ...
            className="relative w-full max-w-md bg-white h-full shadow-2xl z-10 flex flex-col justify-between text-black"
          >
            {/* Drawer header */}
            <div className="flex items-center justify-between px-6 py-5 border-b border-neutral-100">
              ...
              <motion.button
                ...
                whileTap={{ scale: 0.9 }}
                onClick={() => setCartOpen(false)}
                className="p-1 text-neutral-400 hover:text-black transition-colors focus:outline-none cursor-pointer"
            ...
            <div className="flex-1 overflow-y-auto px-6 py-4 divide-y divide-neutral-100">
              {cart.length === 0 ? (
                <motion.div
                  ...
                  className="h-full flex flex-col items-center justify-center text-center py-16 text-neutral-400"
                >
                  ...
                  <motion.button
                    ...
                    whileTap={{ scale: 0.96 }}
                    onClick={() => setCartOpen(false)}
                    className="mt-6 text-[10px] uppercase tracking-[0.2em] font-medium px-5 py-2.5 bg-black text-white hover:bg-neutral-800 transition-colors cursor-pointer"
                  >
                    Continue Shopping
                  </motion.button>
                </motion.div>
                ...
                <AnimatePresence initial={false}>
                  {cart.map((item, index) => {
                    ...
                    return (
                      <motion.div
                        key={`${item.productId}-${item.sizeId}-${item.colorName}-${index}`}
                        layout
                        initial={{ opacity: 0, y: 12 }}
                        animate={{ opacity: 1, y: 0 }}
                        exit={{ opacity: 0, x: 20, height: 0, marginBottom: 0, paddingBottom: 0, paddingTop: 0 }}
                          ...
                          <div>
                            <div className="flex justify-between items-start gap-2">
                              <button
                                ...
                                className="text-left text-[11px] uppercase tracking-wider font-medium text-black hover:opacity-70 transition-opacity line-clamp-1 cursor-pointer"
                              >
                                {product.name}
                              </button>
                              <motion.button
                          ...
                          <div className="flex justify-between items-end mt-3">
                            ...
                            <div className="flex items-center border border-neutral-200">
                              <motion.button
                                ...
                                className="px-2 py-1 text-neutral-500 hover:text-black transition-colors cursor-pointer"
            ...
            {cart.length > 0 && (
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                className="border-t border-neutral-100 p-6 bg-white space-y-4"
              >
                ...
                <motion.button
                  ...
                  className="w-full bg-black text-white hover:bg-neutral-800 transition-colors py-3.5 px-4 flex items-center justify-between text-[11px] uppercase tracking-[0.2em] font-medium cursor-pointer"
                >
