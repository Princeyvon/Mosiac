...
export const ProductGrid: React.FC = () => {
  ...
  const filteredProducts = useMemo(() => {
    ...
    return visible.filter(p => {
      if (filter === 'on-sale') {
        return (
          p.tags?.some(t => t.toLowerCase().includes('sale')) ||
          p.fromPrice < 1500 ||
          p.tags?.includes('Sample Sale')
        );
      }
      if (filter === 'featured') {
        return p.featured || p.tags?.some(t => t.toLowerCase().includes('feat'));
      }
      if (filter === 'new') {
        return p.newArrival || p.tags?.some(t => t.toLowerCase().includes('new'));
      }
      return p.tags?.some(t => t.toLowerCase().replace(/\s+/g, '-') === filter);
    });
      ...
      <div className="max-w-[1920px] mx-auto px-3 sm:px-6 py-6 sm:py-8">
        {filteredProducts.length === 0 ? (
          <div className="py-20 text-center">
            <p className="text-xs uppercase tracking-[0.2em] text-neutral-400 font-sans mb-3">
              No editions found under this filter
            </p>
            <button
              type="button"
              onClick={() => setActiveFilter('all')}
              className="text-xs uppercase tracking-[0.18em] underline text-neutral-800 hover:text-black cursor-pointer"
            >
              View All Works
            </button>
          </div>
        ) : (
          /* Responsive Grid: 6 on desktop (2 on mobile) vs 4 on desktop (1 on mobile) */
          <div
